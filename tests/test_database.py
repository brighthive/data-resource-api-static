...
app_environment = str(os.getenv('APP_ENV')).upper()
if app_environment == 'INTEGRATION':
...
def setup_test_database():
    if app_environment != 'INTEGRATION':
        ...
        database_fixture.start_database(True)
...
def _():
    ...
    def _():
        # skip loading the database if in integration mode
        if app_environment != 'INTEGRATION':
            ...
            expect(len(result)).to(
                equal(len(data['program_potential_outcomes'])))
